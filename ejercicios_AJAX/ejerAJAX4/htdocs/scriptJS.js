...
listadoProvincias.addEventListener("change", (e) =>{
    let provincia = listadoProvincias.options[listadoProvincias.selectedIndex].value;
    //console.log(provincia) muestra el codigo de la provincia seleccionada

    fetch('http://ejerajax4.loc/cargaMunicipiosXML.php', {method: 'POST', headers: {"Content-type": "application"}, body: provincia})
    .then(response => {
        if(response.ok){
            return response.text();
        }
